...
def load_data(filename):
    ...
    with open(filename, 'r') as f:
        ...
        if filename == 'datavn/train':
            joblib.dump(train, 'model/train_bow_5500.pkl')
        else:
            joblib.dump(train, 'model/test_bow_5500.pkl')
    return train


def svm():
    train = load_model('model/train_bow_5500.pkl')
    if train is None:
        train = load_data('datavn/train')

    vectorizer = load_model('model/vectorizer_bow_5500.pkl')
    if vectorizer == None:
    ...
    test = load_model('model/test_bow_5500.pkl')
    if test is None:
    ...
    joblib.dump(vectorizer, 'model/vectorizer_bow_5500.pkl')
    ...
    clf = load_model('model/bow_5500.pkl')
    if clf is None:
        t0 = time.time()
        clf = SVC(kernel='rbf', C=1000, class_weight='balanced')
        clf.fit(X_train, y_train)
        joblib.dump(clf, 'model/bow_5500.pkl')
    ...
    print "-----------------------"
    clf2 = load_model('model/bow2_5500.pkl')
    if clf2 is None:
        t2 = time.time()
        clf2 = SVC(kernel='rbf', C=1000, class_weight='balanced')
        clf2.fit(X_train, y_train2)
        joblib.dump(clf2, 'model/bow2_5500.pkl')
