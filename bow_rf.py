...
def load_data(filename):
    ...
    with open(filename, 'r') as f:
        ...
        if filename == 'datavn/train':
            joblib.dump(train, 'model_rf/train_bow.pkl')
        else:
            joblib.dump(train, 'model_rf/test_bow.pkl')
    return train


def rf():
    train = load_model('model_rf/train_bow.pkl')
    if train is None:
        train = load_data('datavn/train')

    vectorizer = load_model('model_rf/vectorizer_bow.pkl')
    if vectorizer == None:
    ...
    test = load_model('model_rf/test_bow.pkl')
    if test is None:
    ...
    joblib.dump(vectorizer, 'model_rf/vectorizer_bow.pkl')
    ...
    clf = load_model('model_rf/bow.pkl')
    if clf is None:
        t0 = time.time()
        clf = RandomForestClassifier(n_estimators=100)
        clf.fit(X_train, y_train)
        joblib.dump(clf, 'model_rf/bow.pkl')
    ...
    print "-----------------------"
    clf2 = load_model('model_rf/bow2.pkl')
    if clf2 is None:
        t2 = time.time()
        clf2 = RandomForestClassifier(n_estimators=100)
        clf2.fit(X_train, y_train2)
        joblib.dump(clf2, 'model_rf/bow2.pkl')
