...
def load_data(filename):
    ...
    with open(filename, 'r') as f:
        ...
        if filename == 'datavn/train':
            joblib.dump(train, 'model_pos/train_pos1.pkl')
        else:
            joblib.dump(train, 'model_pos/test_pos1.pkl')
    return train


def svm():
    train = load_model('model_pos/train_pos1.pkl')
    if train is None:
        train = load_data('datavn/train')

    vectorizer = load_model('model_pos/vectorizer_pos1.pkl')
    if vectorizer == None:
        vectorizer = TfidfVectorizer(ngram_range=(1, 1), max_df=0.7, min_df=2, max_features=1000)
    test = load_model('model_pos/test_pos1.pkl')
    if test is None:
    ...
    joblib.dump(vectorizer, 'model_pos/vectorizer_pos1.pkl')
    ...
    clf = load_model('model_pos/pos1.pkl')
    if clf is None:
        t0 = time.time()
        clf = SVC(kernel='rbf', C=1000)
        clf.fit(X_train, y_train)
        joblib.dump(clf, 'model_pos/pos1.pkl')
