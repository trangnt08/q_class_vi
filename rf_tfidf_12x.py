...
def load_data(filename):
    ...
    with open(filename, 'r') as f:
        ...
        if filename == 'datavn/train':
            joblib.dump(train, 'model_pos/train_rf_pos12x.pkl')
        else:
            joblib.dump(train, 'model_pos/test_rf_pos12x.pkl')
    return train


def svm():
    train = load_model('model_pos/train_rf_pos12x.pkl')
    if train is None:
        train = load_data('datavn/train')

    vectorizer = load_model('model_pos/vectorizer_rf_pos12x.pkl')
    if vectorizer == None:
        vectorizer = TfidfVectorizer(ngram_range=(1, 2), max_df=0.7, min_df=2, max_features=1000)
    test = load_model('model_pos/test_rf_pos12x.pkl')
    if test is None:
    ...
    joblib.dump(vectorizer, 'model_pos/vectorizer_rf_pos12x.pkl')
    ...
    clf = load_model('model_pos/rf_pos12x.pkl')
    if clf is None:
        t0 = time.time()
        clf = RandomForestClassifier(n_estimators=100)
        clf.fit(X_train, y_train)
        joblib.dump(clf, 'model_pos/rf_pos12x.pkl')
    ...
    print "-----------------------"
    clf2 = load_model('model_pos/rf_pos_fine12x.pkl')
    if clf2 is None:
        t2 = time.time()
        clf2 = RandomForestClassifier(n_estimators=100)
        clf2.fit(X_train, y_train2)
        joblib.dump(clf2, 'model_pos/rf_pos_fine12x.pkl')
