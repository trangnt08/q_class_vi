...
def load_data(filename):
    ...
    with open(filename, 'r') as f:
        ...
        if filename == 'datavn/train':
            joblib.dump(train, 'model2/train_tfidf1.pkl')
        else:
            joblib.dump(train, 'model2/test_tfidf1.pkl')
    return train


def svm():
    train = load_model('model2/train_tfidf1.pkl')
    if train is None:
        train = load_data('datavn/train')

    vectorizer = load_model('model2/vectorizer_tfidf1.pkl')
    if vectorizer == None:
        vectorizer = TfidfVectorizer(ngram_range=(1, 1), max_df=0.7, min_df=2, max_features=1000)
    test = load_model('model2/test_tfidf1.pkl')
    if test is None:
    ...
    joblib.dump(vectorizer, 'model2/vectorizer_tfidf1.pkl')
    ...
    clf = load_model('model2/tfidf1.pkl')
    if clf is None:
        t0 = time.time()
        clf = SVC(kernel='rbf', C=1000)
        clf.fit(X_train, y_train)
        joblib.dump(clf, 'model2/tfidf1.pkl')
    ...
    print "-----------------------"
    clf2 = load_model('model2/tfidf_fine.pkl')
    if clf2 is None:
        t2 = time.time()
        clf2 = SVC(kernel='rbf', C=1000)
        clf2.fit(X_train, y_train2)
        joblib.dump(clf2, 'model2/tfidf_fine.pkl')
